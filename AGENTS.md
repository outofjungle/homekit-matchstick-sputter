...
```bash
git add <files>
python3 scripts/commit.py "WIP: <brief description of change>"
```
**IMPORTANT: Before every commit:**
- Check if `docs/img/pairing_qr.png` was modified (generated by `make generate-pairing`)
- If changed, include it in the commit with the related changes
- Run `git status` and check for untracked, non-gitignored files. If any exist, **ask the user** whether they should be included — they may be files that were created but dropped from context (e.g. when context resets mid-session). Be vigilant; missing files are easy to overlook.

**Do NOT use `git commit` directly** — always use `scripts/commit.py` to ensure version tracking.

### Firmware Version (FIRMWARE_PATCH bump)
The firmware version is `FIRMWARE_MAJOR.PAIRING_CONFIG_ID.FIRMWARE_PATCH` (e.g. `1.12.0`).

**On every commit**, stage files with `git add`, then run:
```bash
python3 scripts/commit.py "WIP: description of change"
```
